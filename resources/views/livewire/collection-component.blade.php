            ...
            @foreach ($this->collections as $collection)
                <a class="group block overflow-hidden"
                    ...
                    <div class="w-full aspect-square overflow-hidden">
                        <img src="{{ $collection['image'] }}" alt="{{ $collection['name'] }}" class="size-full object-cover group-hover:scale-105 transition-transform duration-300">
                    </div>
                    <div class="relative space-y-4 bg-white py-6">
                        <h2>Eau De Parfume</h2>
                        <h3 class="font-semibold text-gray-900 text-lg">{{ $collection['name'] }}</h3>
                        <p class="text-gray-900 text-sm">IDR {{ number_format($collection['price'], 0) }}</p>
